...
- Aggiornato `analysis_options.yaml` con regole di lint aggiuntive e gestione degli errori.
- Refactor della funzione `main`: uso di `async`/`await` per l'inizializzazione della formattazione delle date.
- Refactor del widget `Expenses` per migliorare la gestione del `mainContent` e semplificare il metodo `build`.
- Refactor del layout del widget `Expenses` per adattarsi alla larghezza dello schermo (logica responsive).
- Abilitato l'inserimento di una nuova spesa anche in modalità landscape.
